...
def handle_image(bytes_data):
    ...
    print("payload len 1 " + str(payload_len))
    if payload_len == 126:
        print("payload len 2 " + str(read_bytes(binary_data,16,32)))
        return read_bytes(binary_data,16,32)
    elif payload_len == 127:
        print("payload len 3 " + str(read_bytes(binary_data,16,80)))
        return read_bytes(binary_data,16,80)
    elif payload_len == 0:
        print("payload len 0")
        print(respond_message)
        return 0
...
def create_websocket_format(payload_data, payload_len, extend_payload_len):
    # print(bin(int.from_bytes(payload_data,byteorder='big'))[2:])
    # print(len(bin(int.from_bytes(payload_data,byteorder='big'))[2:]))
    # print(str(payload_data))
    # print("payload len " + str(payload_len))
    # print("extend payload len " + str(extend_payload_len))
        ...
        #try:
            while(True):
                ...
                if self in self.client_sockets:
                    ...
                    print("handling socket request")

                    length = handle_image(received_data)
                    
                    data_length = len(received_data)
                    print("data length " + str(data_length))
                    is_not_all = True
                    if len(received_data) >= 2**10:
                        while(is_not_all):
                            incoming = self.request.recv(1024)
                            received_data += incoming
                            data_length += len(incoming)
                            if(length < data_length or len(incoming) == 0):
                                is_not_all = False
                    ...
                    if path == '/':
                        ...
                        print("accept response " + str(accept_response))
                        print("hash " + str(hash_accept_response.digest()))
                        print("base64 " + str(base64_accept_response))
                        websocket_response = message_101 + base64_accept_response + content.encode()
                        print(websocket_response.decode())
                        self.client_sockets.append(self)
                        print("updated client_sockets" + str(self.client_sockets))
                        ...
                        for x in mycol.find():
                            ...
                            # print("x " + str(payload_data))
                            # print("x " + str(payload_len))
                            # print("x " + str(extend_payload_len))
